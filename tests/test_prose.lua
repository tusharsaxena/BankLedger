...
-- sweep of that day found 45 lines of British spelling across 24 files here -- a mark decoder's
-- test names, `behaviour` in three module headers, `colour` through the browser -- and nothing in
-- the repository could see one of them: `luacheck` does not read English, and every other suite here
-- reads behavior. Four sibling repos already carried this gate and this one did not, which is the
-- whole of why the drift was only ever found by a sweep somebody remembered to run.
--
