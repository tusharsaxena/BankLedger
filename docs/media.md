...
bytes arrive inside the LibKa0s payload (`libs/LibKa0s/media/fonts/JetBrainsMono-Regular.ttf` — for
*which* release of it, read the provenance line in the root [`CLAUDE.md`](../CLAUDE.md), which is
the single machine-read answer; a version restated here would not move with the copy, and this
sentence carried a stale one until it was cut), `core/MediaSetup.lua` publishes the `NS.MediaFont`
seam that resolves the path, and
