...
| `core/ItemSetup.lua:67` `LoadItem` | `C_Timer.After(0.4, cb)` | **One-shot**, and only when the caller passes a callback. Two callers: the bank-scan quality gate at `modules/Ledger.lua:427` passes none, so that path requests the item and arms **no timer at all**; `settings/Panel.lua:229` passes one, to relabel a filter-list row once the client has cached the name — an options-panel repaint the player opened by hand, not a combat path. |
