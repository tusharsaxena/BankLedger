...
| `modules/Ledger.lua` — `GUILD_DATA_EVENTS` | `GUILDBANKBAGSLOTS_CHANGED` -> `OnGuildBankData` | Only ever fires while a guild bank frame is open, which is out of combat. |
