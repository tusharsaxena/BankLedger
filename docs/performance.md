...
|---|---|---|
| `core/BankLedger.lua` `NS.StandUp` | `PLAYER_ENTERING_WORLD` -> `OnEnterWorld` | Two field checks. Latched by `NS.State.cleanupDone` (set when the prune runs) and `NS.State.cleanupPending` (while it is armed), so it does its work once per session. |
| `core/BankLedger.lua` `NS.StandUp` | `PLAYER_REGEN_DISABLED`, `PLAYER_REGEN_ENABLED` -> `OnCombatChanged` -> `NS.Util.ApplyVisibility` | **On the combat EDGE, twice a fight, never inside one.** One `settings.visibility` read, at most one `InCombatLockdown()` call, then a fixed two-entry loop over `NS.Browser` / `NS.SessionWindow` that hides or re-shows a window the rule itself took (`core/State.lua`'s `hiddenByVisibility`). No allocation, no scan, no timer. On the shipped default `always`, `Util.VisibilityAllows` answers before it reads the combat state and the loop finds nothing to do. |
| `core/BankLedger.lua` `OnEnterWorld` | `addon:ScheduleTimer(…, 5)` -> `Database:PruneOld` | **One-shot**, not a ticker. Fires five seconds after the first login/zone, off the boot spike. An AceTimer handle (`NS.State.cleanupPending`), so the stand-down's `CancelAllTimers` cancels it; the once-per-session latch `cleanupDone` is set only when the prune runs, so a stand-down inside the window postpones the prune to the next `PLAYER_ENTERING_WORLD`. |
| `core/ItemSetup.lua:67` `LoadItem` | `C_Timer.After(0.4, cb)` | **One-shot**, and only when the caller passes a callback. One caller: the bank-scan quality gate at `modules/Ledger.lua:427` passes none, so that path requests the item and arms **no timer at all**. The Filters tab's lists no longer call it: their vendored LibKa0s `IdList` asks `LibKa0s-Item-1.0`'s own `LoadItem` (the same one-shot), and since the v1.35.0 re-cut it batches, so one render's uncached ids share **one** 0.4 s check and at most one repaint, each id asked for at most five times. An options-panel repaint the player opened by hand, not a combat path. |
| `modules/Ledger.lua` — `OPEN_EVENTS` / `CLOSE_EVENTS` | `BANKFRAME_OPENED`, `BANKFRAME_CLOSED` | **Cannot fire in combat**: a bank frame is an out-of-combat NPC interaction. These, with `GuildBankFrame`'s `OnShow`/`OnHide` hooks for the guild bank (whose `GUILDBANKFRAME_*` events never fire and are not registered), are what set `NS.State.openContext`, the gate everything else reads. |
...
| `modules/Browser.lua`, `modules/SessionWindow.lua` | `PLAYER_LOGOUT` on each window's private bus target (`NS.NewBusTarget()`) | Saves the window's view state. Fires once, at logout. |
