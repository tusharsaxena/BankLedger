...
|---|---|---|
| `core/BankLedger.lua` `OnEnable` | `PLAYER_ENTERING_WORLD` -> `OnEnterWorld` | Two field checks. Latched by `NS.State.cleanupDone` (set when the prune runs) and `NS.State.cleanupPending` (while it is armed), so it does its work once per session. |
