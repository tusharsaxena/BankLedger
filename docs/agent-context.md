...
`core/ defaults/ locales/ modules/ settings/`, 663 headless test cases.
...
3. **Green gate before every commit.** `lua tests/run.lua` (663 cases, 0 failed) and `luacheck .`
...
tests/
  ...
  test_<module>.lua  -- 18 suites, 663 cases (test_harness.lua guards the suite list and TOC order)
```
...
- [x] `tests/` harness present; `lua tests/run.lua` is **green** (663/663); behavior is covered test-first (testing).
