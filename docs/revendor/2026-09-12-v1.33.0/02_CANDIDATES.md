...
- **The font preload (Options minor 17).** **A no-op here.** The Options descriptor supplies no `getLSM` (`settings/OptionsSetup.lua:97` says why: no row is LSM-backed), and the preload returns at `type(d.getLSM) ~= "function"` before it builds a frame. The OnShow hook `O.CreatePanel` installs runs and returns.
- **The `count` docstrings (Options 17, Slash 9).** Comments only; nothing to adopt.
- **Kit revision 18, the `OnProfileCopied` key.** **Not reached.** BankLedger's harness replaces the kit's AceDB with one that fires no callbacks (`tests/wow_mock.lua`), and the addon has no copy handler.
