...
|---|---|---|---|
| B1 — #29 AceEvent event half on an embed | **decline (not now)** | `tests/wow_mock.lua:712` replaces the kit Embed wholesale, so kit 16's event half never runs here. Lines :714-716 are **not** redundant. Deleting them removes the headless event half: the characterization run stayed at 850/850, but only because the three production call sites are existence-guarded and skip silently. Adopting means wrapping `base`'s Embed, which is a harness migration. | proposed, not filed |
| B2 — #30 `Printf` beside `Print` | **decline (not now)** | `tests/wow_mock.lua:679` replaces the kit `NewAddon` wholesale for the `__badEvents` raise and the honored-cancel timers, so the kit's `Printf` stamp cannot reach. Nothing is exposed today, because the addon defines no `NS.Printf`. Adopting means wrapping `base.NewAddon`, which is a harness migration. | proposed, not filed |
