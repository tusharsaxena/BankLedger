...
```
diff -r --strip-trailing-cr <tag>/LibKa0s BankLedger/libs/LibKa0s   # 1 file: OptionsCompose.lua
diff -r --strip-trailing-cr <tag>/testkit BankLedger/tests/_kit     # empty
```
...
Majors reached from this addon's own source: Core DebugLog Env Item Media Options Perf Pool Slash Widgets 
