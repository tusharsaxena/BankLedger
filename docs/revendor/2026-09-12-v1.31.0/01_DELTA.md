...
`git -C ../LibKa0s archive v1.31.0 LibKa0s testkit | tar -x -C <scratch>/`. The tag is `30db4ed` on
the library's `feat/2026-09-12-v1.31.0` branch, which is not merged to the library's `master`. That
...
Two files moved, both in the Options major. **No cross-major skew.** After the copy the payload
