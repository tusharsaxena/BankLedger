...
| `core/PoolSetup.lua` | The **LibKa0s-Pool-1.0 seam**: publishes `NS.Pool` with `New`, `Acquire(pool, factory)`, `ReleaseAll(pool, before)` and `Counts(pool)`, and the same pool written out in full as the degraded fallback. It replaced **three** hand-rolled pools in this addon — the generic one in `modules/InsightsWidgets.lua` and the inline row pools in `modules/LedgerTable.lua` and `modules/SessionWindow.lua`. All three of this addon's were correct; LootHistory's fourth copy of the same eleven lines was not, and leaked a frame per chart element per re-render. Two addons wrote the same code and one got the second half wrong — that is the case for the library, and it is why `Counts` exists: a pool that leaks answers `0, 0` where a correct one answers `n, 0`, which is the only way recycling is visible to a test. `ReleaseAll`'s optional `before` hook is what lets one function cover `W.ReleasePanels`, whose panels each own a nested `_rows` pool. **`ReleaseAll` parks the active set backward**, and as of Pool minor 3 that is the library's published contract rather than an incidental loop direction: `Acquire` pops the free list from the END, so releasing the last rank first leaves rank 1 on top and the next `Acquire` hands each widget back to the rank it already held. Walking forward alternates that mapping on every render, which matters here because `modules/InsightsWidgets.lua` assigns `PaletteColor` **by rank** — a widget that changes rank between passes changes color between passes. The degraded fallback mirrors the direction for the same reason: a shim that quietly contradicts a published contract is worse than no shim, because it only misbehaves on the installs nobody tests. |
| `core/State.lua` | Runtime-only state: the open frame, the last snapshot, the session debug flag, the test dataset, and `hiddenByVisibility` — the set of windows the **General visibility** rule itself took off screen, so the transition back re-shows exactly those and never one the player had closed. Never persisted. |
| `core/Util.lua` | Player key, path splitting, date/money/byte formatting, the wowhead URL builder, the pooled-row tint (`RowTintAlpha`, `ApplyRowTint`, `RefreshRowTint`) — the single clamped read of `settings.rowStripeAlpha` / `settings.rowHoverAlpha` for both pooled tables, so the two promoted literals are not spelled out four times — and the **Master controls seam** the composed tab is honored through: `ApplyMasterFrame` (master scale, the clamped master alpha and `Lock frame`, applied to one frame), `ApplyMasterChrome` (the same across the ledger window, the session window and the export modal), `VisibilityAllows` / `ApplyVisibility` (the four-mode `settings.visibility` rule and the remembered re-show) and `ResetWindowPositions` (the one body behind both the **Reset position** button and the page's **Defaults**). The secret-safe chat printer moved to `core/CoreSetup.lua` when LibKa0s was adopted; every name it published is unchanged. |
| `core/BankLedger.lua` | AceAddon registration, the message bus, `NS.NewBusTarget`, `OnInitialize` / `OnEnable`, and `OnCombatChanged` — both combat edges routed into `NS.Util.ApplyVisibility`, which is what makes **General visibility**'s two combat modes an answer that changes without anything being clicked. |
...
| `settings/Schema.lua` | The schema table (the single source for panel, slash and defaults) and `NS.COMMANDS`. `group` names a **tab**, and declaration order is tab order — Master controls, Capture, Interface, History, then the two renderer-only rows (`S.BespokeRows`) that name the Blacklist and Whitelist tabs. `S.MASTER_SPEC` / `S.MASTER_DECOR` / `S:ComposeMaster` are the Master controls tab (`options-ui-§15`): the library composes the canonical eight and this file supplies only the stored paths (`keys`) and the reactions. `S:Set` is the **single write seam**: it validates, writes, emits the one debug trace, runs the row's `onChange`, and repaints an open settings panel (options-ui-§1, options-ui-§11) — so a slash write and a panel widget take exactly the same path. |
| `settings/Slash.lua` | The **LibKa0s-Slash-1.0 seam**, plus what stays the host's: AceConsole registration, the four confirm dialogs, `Sl:Version`, and the full reset. The dispatcher, the help renderer and the `list`/`get`/`set`/`reset`/`resetall` CLI are the library's. Supplies `groupKey` (this schema groups by `group`, not `page`), a `format` hook for the set-typed `excludedStores` row, and a `parse` override that refuses a chat edit of that row by name. Wraps `CliResetAll` so the two carve-outs with no Schema widget — the filter lists and the saved ledger view — are still reset. **`Sl:ResetEverything` is the destructive form and is wholesale**: this addon has no `profile` section, so `options-ui-§12` translates to emptying `db.global` **in place** (modules capture that table at load) and merging `NS.defaults.global` back — never a list of things to clear, which is the shape that quietly stops covering the store. |
...
| `settings/Panel.lua` | What did **not** generalize: the inverted store grid, the History tab's storage read-out, the two item-id list tabs, the landing-page body, `P:Diagnose` and the `P:Batch` refresh coalescer. Registers **two** pages with the library — the landing page and General — and lets it own the shell, the makers, the flow engine and the render timing. General is tabbed (`options-ui-§13`) through `O.RenderTabbedSchema` on the rows' own `group` field. **The Filters sub-page is gone**: it held no schema rows, so its two lists are two of General's tabs now and the registration was removed rather than left empty. Every bespoke block is an `afterGroup` hook rather than a page-renderer call — a tab click re-renders the schema alone, so anything the body drew would survive exactly one render. |

#### Load order is load-bearing

The TOC's file order is not cosmetic in six places, and the comments in `BankLedger.toc` say so at
each one. Two more files carry a position note that deliberately says the opposite — `core/EnvSetup.lua`
(listed below for that reason) and `core/PoolSetup.lua` — and neither is one of the six:
