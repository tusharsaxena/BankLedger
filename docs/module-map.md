...
| `core/Util.lua` | Player key, path splitting, date/money/byte formatting, the wowhead URL builder, and the pooled-row tint (`RowTintAlpha`, `ApplyRowTint`, `RefreshRowTint`) — the single clamped read of `settings.rowStripeAlpha` / `settings.rowHoverAlpha` for both pooled tables, so the two promoted literals are not spelled out four times. The secret-safe chat printer moved to `core/CoreSetup.lua` when LibKa0s was adopted; every name it published is unchanged. |
