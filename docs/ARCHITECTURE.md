...
| `settings/Panel.lua` | What did **not** generalise: the inverted store grid, the Storage section, the whole Filters page, the landing-page body, `P:Diagnose`, the `P:Batch` refresh coalescer, and the `OnCommit`/`OnDefault`/`OnRefresh` canvas contract the library does not stamp. Registers three pages with the library and lets it own the shell, the makers, the flow engine and the render timing. |
